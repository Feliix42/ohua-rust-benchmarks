...
pub fn calculate(options: Vec<OptionData>) -> Vec<f32> {
    ...
    let (results_0_1_0_tx, results_0_1_0_rx) = std::sync::mpsc::channel();
    let (results_0_0_1_tx, results_0_0_1_rx) = std::sync::mpsc::channel();
    let (ctrl_0_0_tx, ctrl_0_0_rx) = std::sync::mpsc::channel::<(_, _)>();
    let (d_0_0_tx, d_0_0_rx) = std::sync::mpsc::channel::<OptionData>();
    ...
    tasks.push(Box::new(move || -> _ {
        ...
        loop {
            ...
            let futures_0 = {
                ...
                let work = async move { tx.send(calculate_black_scholes(var_1)).unwrap() };
                rt.spawn(work);
                rx
            };
