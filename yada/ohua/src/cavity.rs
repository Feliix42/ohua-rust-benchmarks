...
impl Cavity {
    ...
    pub fn new(_mesh: &Mesh, node: Element, original: Triangle) -> Option<Self> {
        let mut frontier = VecDeque::new();
        let mut previous_nodes = Vec::new();
        let new_nodes = Vec::new();

        // println!("Starting with: {}", node);
        let original_center = original;
                ...
                Element::T(ref t) => {
                    ...
                    for neighbor in neighbors {
                        if let Err(other) = self.expand(curr, *neighbor) {
                            ...
                            *self = Self::new(mesh, other, self.original_center).unwrap();
                        }
                    }
                }
                Element::E(ref e) => {
