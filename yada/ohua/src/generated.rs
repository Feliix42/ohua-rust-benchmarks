...
pub fn run_refine(mesh: Mesh) -> Mesh {
    ...
    let (is_not_empty_0_0_0_tx, is_not_empty_0_0_0_rx) = std::sync::mpsc::channel();
    let (remaining_work_0_0_0_0_tx, remaining_work_0_0_0_0_rx) =
        std::sync::mpsc::channel::<Vec<Triangle>>();
    ...
    let (mesh_0_0_3_tx, mesh_0_0_3_rx) = std::sync::mpsc::channel::<Mesh>();
    let (a_0_0_tx, a_0_0_rx) = std::sync::mpsc::channel::<Mesh>();
    let (bad_0_0_0_tx, bad_0_0_0_rx) = std::sync::mpsc::channel();
    let (bad_0_n_0_0_0_tx, bad_0_n_0_0_0_rx) = std::sync::mpsc::channel::<Vec<Triangle>>();
    let (computed_0_0_1_tx, computed_0_0_1_rx) = std::sync::mpsc::channel::<Vec<Option<Cavity>>>();
    let (ctrl_2_0_tx, ctrl_2_0_rx) = std::sync::mpsc::channel::<(_, _)>();
    let (m_0_0_1_tx, m_0_0_1_rx) = std::sync::mpsc::channel::<Arc<Mesh>>();
    ...
    let (futures_0_tx, futures_0_rx) =
        std::sync::mpsc::channel::<std::sync::mpsc::Receiver<Option<Cavity>>>();
    let (cav_0_0_0_tx, cav_0_0_0_rx) = std::sync::mpsc::channel();
    let (computed_0_1_0_tx, computed_0_1_0_rx) = std::sync::mpsc::channel::<Vec<Option<Cavity>>>();
    let (mesh_0_0_2_0_tx, mesh_0_0_2_0_rx) = std::sync::mpsc::channel::<Mesh>();
    let (rest_0_0_0_tx, rest_0_0_0_rx) = std::sync::mpsc::channel();
    let (remaining_work_0_1_0_tx, remaining_work_0_1_0_rx) = std::sync::mpsc::channel();
    let (mesh_0_0_1_0_tx, mesh_0_0_1_0_rx) = std::sync::mpsc::channel::<Mesh>();
    ...
    tasks.push(Box::new(move || -> _ {
        ...
        loop {
            ...
            let futures_0 = {
                ...
                let work = async move { tx.send(compute_cavity(var_1, var_2)).unwrap() };
                rt.spawn(work);
                rx
            };
