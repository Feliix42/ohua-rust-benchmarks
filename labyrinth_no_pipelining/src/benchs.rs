...
pub struct Maze {
    ...
    pub unmappable_paths: Arc<Vec<(Point, Point)>>,
}
...
impl Maze {
    ...
    pub fn init(dimensions: Point) -> Self {
        Maze {
            ...
            unmappable_paths: Arc::new(Vec::new()),
        }
    }
    ...
    pub fn update(
        &mut self,
        path_list: Vec<Option<Path>>,
        retry_sender: &std::sync::mpsc::Sender<usize>,
    ) -> Vec<Option<(Point, Point)>> {
        let paths = Arc::get_mut(&mut self.paths).unwrap();
        let grid = Arc::get_mut(&mut self.grid).unwrap();

        let mut failed = Vec::new();
        for path in path_list {
            if let Some(path) = path {
                if path_available(&grid, &path) {
                    for pt in &path.path {
                    ...
                    failed.push(Some((path.start, path.end)));
                }
            }
        }
