...
pub fn run(g: Graph, preflow: PreflowPush) -> Graph {
    ...
    let (graph_0_0_3_tx, graph_0_0_3_rx) = std::sync::mpsc::channel::<Graph>();
    let (d_0_0_tx, d_0_0_rx) = std::sync::mpsc::channel::<Graph>();
    let (initial_0_0_0_tx, initial_0_0_0_rx) = std::sync::mpsc::channel::<HashSet<NodeID>>();
    let (g2_0_0_1_tx, g2_0_0_1_rx) = std::sync::mpsc::channel::<Arc<Graph>>();
    let (ctrl_2_0_tx, ctrl_2_0_rx) = std::sync::mpsc::channel::<(_, _)>();
    let (results_0_0_1_tx, results_0_0_1_rx) =
    ...
    let (e_0_0_tx, e_0_0_rx) = std::sync::mpsc::channel::<Arc<Graph>>();
    let (futures_0_tx, futures_0_rx) = std::sync::mpsc::channel();
    let (result_0_0_0_tx, result_0_0_0_rx) = std::sync::mpsc::channel::<(u64, (NodeID, Graph))>();
    ...
    tasks.push(Box::new(move || -> _ {
        loop {
            ...
            let var_2 = preflow; //preflow_0_0_0_rx.recv()?;
