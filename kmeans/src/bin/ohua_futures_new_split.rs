...
fn main() {
    ...
    for r in 0..runs {
        ...
        let input_data = Arc::new(clusters.clone());
...
#[inline(always)]
fn calc_centroids(values: Arc<Vec<Value>>, old_centroids: Arc<Vec<Centroid>>) -> Arc<Vec<Centroid>> {
...
fn unpack_updates(mut values: Vec<(Value, f32)>) -> (Arc<Vec<Value>>, f32) {
    ...
    (Arc::new(new_values), current_delta)
}
