...
// ) -> Maze {
//   let rs = Vec::default();
//   for pair in pairs { let path = find_path(maze.clone(), pair); rs.push(path); }
...
pub fn run(dimensions: Point, pairs: Vec<Option<(Point, Point)>>, max_it: u32) -> Maze {
    ...
    let (maze_0_1_0_0_tx, maze_0_1_0_0_rx) = std::sync::mpsc::channel();
    ...
    let (maze_0_0_1_tx, maze_0_0_1_rx) = std::sync::mpsc::channel::<Maze>();
    ...
    let (rs_0_0_0_0_tx, rs_0_0_0_0_rx) = std::sync::mpsc::channel::<Vec<Option<Path>>>();
    let (maze_0_1_1_tx, maze_0_1_1_rx) = std::sync::mpsc::channel::<Maze>();
    let (rs1_0_0_1_tx, rs1_0_0_1_rx) = std::sync::mpsc::channel::<Vec<Option<(Point, Point)>>>();
    ...
    tasks.push(Box::new(move || -> _ {
        ...
        let finalResult = maze_0_1_0_0_rx.recv()?;
    ...
    tasks.push(Box::new(move || -> _ {
        let s = retry_sx;
        loop {
            let mut var_0 = maze_0_1_1_rx.recv()?;
            let var_1 = rs_0_0_0_0_rx.recv()?;
            let rs1_0_0_1 = var_0.update(var_1, &s);
            rs1_0_0_1_tx.send(rs1_0_0_1)?;
            maze_0_1_0_0_tx.send(var_0)?
        }
    }));
    tasks.push(Box::new(move || -> _ {
        loop {
    ...
    tasks.push(Box::new(move || -> _ {
        loop {
            ...
            let res = {
                let sp = if var_0.len() < 42 { var_0.len() } else { 42 };
    ...
    tasks.push(Box::new(move || -> _ {
        loop {
            let mut renew = false;
            let mut maze_0_0_1_0 = maze_0_0_1_rx.recv()?;
            while !renew {
                let sig = ctrl_2_0_rx.recv()?;
                let count = sig.1;
                for _ in 0..count {
                    let b_0_0 = maze_0_0_1_0.clone();
                    b_0_0_tx.send(b_0_0)?;
                    ()
                }
                let renew_next_time = sig.0;
            ...
            maze_0_1_1_tx.send(maze_0_0_1_0)?;
            ()
        }
    }));
