...
fn main() {
    ...
    for _ in 0..runs {
        ...
        #[ohua]
        let (filled_maze, collision_count) = modified_algos::transact_split10(maze, paths2);
    ...
    if json_dump {
        ...
        let filename = format!(
            "results/ohua_split10-{}-p{}-r{}_log.json",
            dimensions,
            paths.len(),
            runs
        );
...
fn join(
    ...
    mut v9: Vec<Option<Path>>,
    mut v10: Vec<Option<Path>>,
) -> Vec<Option<Path>> {
    v1.append(&mut v2);
    v1.append(&mut v3);
    v1.append(&mut v4);
