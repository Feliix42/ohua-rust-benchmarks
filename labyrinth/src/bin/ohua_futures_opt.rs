...
fn main() {
    ...
    if json_dump {
        ...
        let filename = format!(
            "{}/ohua_futures_opt-{}-p{}-freq{}-t{}-ts{}-r{}_log.json",
            out_dir,
            dimensions,
            paths.len(),
            updates,
...
fn spawn_onto_pool(
    ...
    let mut worklist = vec![Vec::with_capacity(l); taskcount];

    for t_num in 0..taskcount {
        if rest > 0 {
            worklist[t_num] = points.split_off(points.len() - l - 1);
            rest -= 1;
        } else {
            if points.len() <= l {
                worklist[t_num] = points.split_off(0);
            } else {
                worklist[t_num] = points.split_off(points.len() - l);
            }
        }
    }
