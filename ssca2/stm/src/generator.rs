...
impl GraphSDG {
    pub fn threaded_generate(parameters: Parameters) -> Self {
        ...
        for thread_id in 0..parameters.threads {
            handles.push(thread::spawn(move || {
                ...
                let (i_start, i_stop) = create_partition(0, parameters.total_vertices, thread_id, parameters.threads);
        ...
        // ====================================================================
        // ====================================================================
        // TODO(feliix42): CONTINUE HERE with STEP 4
    ...
    pub fn generate(parameters: Parameters) -> Self {
        // thread barriers for synchronization (mixing models here a bit)
        let barrier = Arc::new(Barrier::new(parameters.threads));

        // the variables associated with the graph tuple
        let mut perm_v: Vec<usize> = (0..parameters.total_vertices).into_iter().collect();

        todo!("Call the new thread_generate function here");
        ...
        // NOTE(feliix42): seq, but intertwined with parallel initialization :/
        // generate random clique sizes
        ...
        for i in i_edge_start_counter..i_edge_end_counter {
            if (i + num_edges_placed_in_cliques) >= start_vertex.len() {
                ...
                println!(
                    "    Still want to place {} more elements",
                    i_edge_end_counter - i - 1
                );
                println!(
                    "    Size of `start_vertex`: {} elements",
                    start_vertex.len()
                );
            }
