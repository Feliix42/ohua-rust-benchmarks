...
fn main() {
    let matches = App::new("Ohua Intruder Benchmark")
        ...
        .about(
            "A Rust port of the intruder benchmark from the STAMP collection, implemented in Ohua.",
        )
        .arg(
    ...
    for r in 0..runs {
        ...
        // run the algorithm
        #[ohua]
        let result = intruder_detector(input_data);
...
fn statistics(mut packets: Vec<(DecodedPacket, DetectorResult)>) -> Vec<usize> {
    packets
        .drain(..)
        .filter_map(|(packet, res)| if res == DetectorResult::SignatureMatch { Some(packet.flow_id) } else { None })
        .collect()
}
