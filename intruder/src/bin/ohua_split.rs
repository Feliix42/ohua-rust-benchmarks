...
fn main() {
    let matches = App::new("Ohua Intruder Benchmark")
        ...
        .about(
            "A Rust port of the intruder benchmark from the STAMP collection, implemented in Ohua, using 2 splits on smap.",
        )
        .arg(
    ...
    for r in 0..runs {
        ...
        // run the algorithm
        #[ohua]
        let result = algos::split_detector(input_data);
    ...
    if json_dump {
        ...
        let filename = format!(
            "{}/ohua_split2-n{}-p{}-s{}-pl{}-r{}_log.json",
...
fn split(mut v: VecDeque<DecodedPacket>) -> (VecDeque<DecodedPacket>, VecDeque<DecodedPacket>) {
    let parts = 2;
...
fn combine(
    mut v1: Vec<(DecodedPacket, DetectorResult)>,
    mut v2: Vec<(DecodedPacket, DetectorResult)>,
